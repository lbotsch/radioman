    ...
    onGridRender : function (grid)
    {
        ...
        this.target = new Ext.dd.DropTarget(grid.getEl(),
        {
            ...
            notifyDrop: function(dd, e, data)
            {
                ...
                // Determine whether the drag item came from the library or the grid
                if(dd.grid) {
                	return this.dropFromGrid(dd, e, data);
                } else if(dd.tree) {
                	return this.dropFromTree(dd, e, data);
                } else {
                	return false;
                }
            },
            
            dropFromGrid: function(dd, e, data) {
                // determine the row
            ...
            dropFromTree: function(dd, e, data) {
            	var nodeData = data.node.attributes;
            	if(!nodeData.leaf || !nodeData.title || !nodeData.artist || !nodeData.path) {
            		return false;
            	}
                ...
                // fire the before move/copy event
                if (this.gridDropTarget.fireEvent('beforerowcopy', this.gridDropTarget, data, nodeData, rindex, 123) === false)
                {
                    return false;
                }
                ...
                // update the store
                var ItemRecord = ds.recordType;
                
                var insertIndex = rindex;
                ds.insert(insertIndex, new ItemRecord(nodeData));
                
                // re-select the row(s)
                var sm = this.grid.getSelectionModel();
                if (sm)
                {
                    sm.selectRow(insertIndex);
                }

                // fire the after move/copy event
                this.gridDropTarget.fireEvent('afterrowcopy', this.gridDropTarget, data, nodeData, rindex);
                return true;
            },

            notifyOver: function(dd, e, data)
            {
            	// Handle over from library tree
            	if(dd.tree) {
            		return this.overFromTree(dd, e, data);
            	}
            ...
            overFromTree: function(dd, e, data) {
            	var t = Ext.lib.Event.getTarget(e);
                var rindex = this.grid.getView().findRowIndex(t);
                ...
                if (rindex < 0 || rindex === false) {
                	...
                	if(rowCount > 0) {
                		var lastRow = this.grid.getView().getRow(rowCount - 1);
                		this.currentRowEl = new Ext.Element(lastRow);
                		this.currentRowEl.addClass('grid-row-insert-below');
                	}
